      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((project) => (
            <StaggerItem key={project._id} className="group bg-[#f8f9fa] rounded-4xl p-6 hover:shadow-xl transition-shadow duration-300">
              <Link href={project.slug?.current ? `/projects/${project.slug.current}` : '#'} className="block">
                ...
                <div className="flex flex-wrap gap-3 mb-6">
                  {project.tags?.map((tag, i) => (
                    <span 
                      key={i} 
                      className="px-4 py-1.5 rounded-full text-xs font-bold text-black bg-primary"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex justify-between items-end gap-4">
                  ...
                  <div className="w-12 h-12 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 bg-black text-white group-hover:bg-primary group-hover:text-black group-hover:rotate-45">
