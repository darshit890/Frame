...
interface HeroClientProps {
  ...
  tags?: string[];
  description?: string;
  statsProjectCount?: string;
  statsProjectLabel?: string;
  statsExpCount?: string;
  statsExpLabel?: string;
  statsCustomerCount?: string;
  statsCustomerLabel?: string;
    ...
    <section className="relative min-h-screen bg-[#111] pt-32 pb-20 overflow-hidden font-sans">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-6 mb-20">
            ...
            <FadeIn direction="left" delay={0.4} className="lg:col-span-3 bg-primary rounded-4xl p-6 lg:p-8 flex flex-col justify-between h-auto min-h-[350px] lg:h-[400px] hover:shadow-[0_0_30px_rgba(196,255,0,0.2)] transition-shadow duration-300">
                 <div className="space-y-4 lg:space-y-8 mt-2 lg:mt-4">
                    <div className="border-b border-black/10 pb-4 lg:pb-6">
                        <h3 className="text-4xl lg:text-6xl font-bold text-black mb-1">{statsProjectCount}</h3>
                        <p className="text-black/60 font-semibold text-sm uppercase tracking-wide">{statsProjectLabel}</p>
                    </div>
                     <div className="border-b border-black/10 pb-4 lg:pb-6">
                        <h3 className="text-4xl lg:text-6xl font-bold text-black mb-1">{statsExpCount}</h3>
                        <p className="text-black/60 font-semibold text-sm uppercase tracking-wide">{statsExpLabel}</p>
                    </div>
                 </div>
                 <div className="mt-4 lg:mt-0">
                    <h3 className="text-4xl lg:text-6xl font-bold text-black mb-1">{statsCustomerCount}</h3>
                    <p className="text-black/60 font-semibold text-sm uppercase tracking-wide">{statsCustomerLabel}</p>
                 </div>
            </FadeIn>
        </div>
      </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-primary py-4 overflow-hidden z-20">
        <div className="flex animate-scroll whitespace-nowrap">
          {[1, 2, 3, 4].map((i) => (
              <span key={i} className="inline-flex items-center gap-8 text-white font-bold text-xl px-8 uppercase tracking-wider">
